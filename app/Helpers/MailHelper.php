...
class MailHelper{
    ...
    public static function testMail1(){
        ...
        Attendance::where('punch_out_time',NULL)->whereDate('punch_in_time',date('Y-m-d'))->where('employee_id','172')->update(['missed_punch_out'=>'1','punch_out_time'=>date('Y-m-d H:i:s')]);
        ...
        foreach($attendances as $attendance){
             try{             
                ...
                // $ccList =  (new self)->getHrEmail();
                // array_push($ccList,(new self)->getManagerEmail($attendance->employee_id));
                $ccList = ['satyadeep.neupane@deerwalk.edu.np','samil.shrestha@deerwalk.edu.np','deena.sitikhu@deerwalk.edu.np'];
                
                $mail= Mail::to($attendance->employee->email)
                    ->cc($ccList)
                    ->queue(new MissedPunchOutMail($employee_name));
            }catch(\Exception $e){
                \Log::debug("Test Mail 1 Error Exception Occured");
                \Log::debug($e);
                redirect()->back()->with('error',$e->getMessage());
            }
        }
