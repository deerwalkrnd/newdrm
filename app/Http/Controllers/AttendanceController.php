...
class AttendanceController extends Controller
{
    ...
    public function takeAttendance($employee_id, $request, $state)
    {
        ...
        $employee_shift_time = Employee::select('id', 'shift_id', 'start_time', 'end_time')
        ...
        if (date('D') == 'Sat' || date('D') == 'Sun') {
            ...
            $weekdays = false;
        } else if ($employee_shift_time && $employee_shift_time->shift_id == '2') {
    ...
    public function punchOut(Request $request)
    {
        ...
        $employee_full_name = $employee ? trim($employee->first_name . ' ' . $employee->middle_name . ' ' . $employee->last_name) : '';
        $employee_email = $employee ? $employee->email : (\Auth::user()->employee->email ?? null);
        $today = date('Y-m-d');
        ...
        if ($this->recordRowExists($employee_id) && !$this->hasPunchOut($employee_id)) {
            ...
            if (date('D') == 'Sat' || date('D') == 'Sun') {
                ...
                $min_punch_out_time = $employee_shift_time->end_time;
            } else {
    ...
    public function ForcePunchOut()
    {
        ...
        $attendances = Attendance::whereDate('punch_in_time', date('Y-m-d'))
            ...
            ->update(['punch_out_time' => date('Y-m-d H:i:s'), 'punch_out_ip' => $ip]);
    ...
    public function report(Request $request)
    {
        try {
            ...
            $attendance = $employees->isEmpty() ? collect() : Attendance::where("employee_id", $employees->first()->id)->get()->sortByDesc('id')->take(5);
